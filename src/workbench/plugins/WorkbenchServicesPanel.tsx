...
export function StorageCleanupPanel(props: Props) {
  ...
  return <section className="feature-status-panel workbench-services storage-center" data-testid="opl-storage-cleanup">
    ...
    <section className="storage-intent-section" aria-labelledby="storage-organize-title">
      ...
      <article className="storage-capability-row"><div><strong>{zh ? '对话' : 'Conversations'}</strong><p>{zh ? 'Codex owner 已提供归档、恢复和永久删除。删除前请先归档或导出需要保留的历史；本页释放空间不会触碰对话。' : 'The Codex owner provides archive, restore and permanent deletion. Archive or export history you need before deleting; release-space cleanup does not touch conversations.'}</p></div><span className="storage-state-neutral">{zh ? '在对话详情中管理' : 'Manage in thread details'}</span></article>
      <article className="storage-capability-row"><div><strong>{zh ? '项目与附件' : 'Projects and attachments'}</strong><p>{zh ? '项目目前只是对话分组，没有独立可删除的 owner；当前会话附件也不是 App-owned 数据库。本页不会猜测或级联删除它们。' : 'Projects are currently conversation groupings without an independent delete owner, and current-session attachments are not an App-owned database. This page does not guess or cascade-delete them.'}</p></div><span className="storage-state-neutral">{zh ? '尚无独立 owner' : 'No independent owner yet'}</span></article>
